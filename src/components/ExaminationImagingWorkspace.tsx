...
export function ExaminationImagingWorkspace({
  ...
  return (
    <div className="module-content examination-imaging-content">
      ...
      {ctAiOpen && selectedStudy && <CTAIAnalysisPanel key={`${patient?.backendId ?? 'none'}-${selectedStudy.id}-${selectedSeriesId}`} patientId={patient?.backendId ?? null} study={selectedStudy} seriesId={selectedSeriesId} onClose={() => setCtAiOpen(false)} onRefresh={() => { selectRenderingKind('CALCIFICATION_ONLY'); setRenderingRevision((value) => value + 1) }} />}
