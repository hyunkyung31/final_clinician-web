...
export function XCAReportFinalize({ target, disabled, onBusyChange, onSigned }: { target: XCAReportTarget; disabled: boolean; onBusyChange: (busy: boolean) => void; onSigned: () => void }) {
  ...
  return <section className="xca-report-draft"><h3>최종 PDF · 의료진 서명</h3>
    <p>{import.meta.env.DEV ? '저장된 본문·점수·선택 영상이 PDF에 포함됩니다. 서명 버튼은 로컬 미리보기이며 실제 승인과 PDF에는 반영되지 않습니다.' : '저장된 본문·점수·선택 영상이 PDF에 포함됩니다. 의사 계정의 명시적 검토·재인증 후 서명합니다. 환자 앱에 자동 공개하지 않습니다.'}</p>
    ...
    {preview?.state === 'DRAFT' && !receipt && <><label><input type="checkbox" checked={confirmed} disabled={busy || disabled} onChange={event => setConfirmed(event.target.checked)} /> 이 버전의 PDF 본문·의견·선택 영상과 AI 연구 제한을 확인했고 최종 서명에 동의합니다.</label>
      {import.meta.env.DEV ? <DoctorSignaturePreview resetKey={`${preview.versionId}:${preview.digest}:${confirmed}`} disabled={busy || disabled || !confirmed} /> : <><label>의료진 비밀번호 재인증<input type="password" autoComplete="current-password" value={password} disabled={busy || disabled} onChange={event => setPassword(event.target.value)} /></label>
